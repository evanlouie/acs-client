/**
 * Typings for ACS-Engine based on the Go typings found in d5ec309
 * @author Evan Louie
...
interface IKubernetesConfig {
  ...
  controllerManagerConfig: { [key: string]: string };
  cloudControllerManagerConfig: { [key: string]: string };
  apiServerConfig: { [key: string]: string };
  cloudProviderBackoff: boolean;
  cloudProviderBackoffRetries: number;
  cloudProviderBackoffJitter: number;
  cloudProviderBackoffDuration: number;
...
interface ILinuxProfile {
  ...
  ssh: { pulicKeys: Array<{ keyData: string }> };
